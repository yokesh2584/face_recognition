...
const RegisterUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If face already exists, show error
      if (verifyResponse.data.faceExists) {
        setMessage({
          text: "This face is already registered with another account. Please contact an administrator.",
          type: "danger",
        });
        setLoading(false);
        return;
      ...
      // If face is unique, proceed with registration
      const response = await axios.post("http://localhost:5000/api/register", {
        ...userData,
        image: capturedImage,
      });
      ...
      setMessage({
        text: `Student registered successfully with ID: ${response.data.user_id}`,
        type: "success",
      });

      // Reset form
      setUserData({ name: "", email: "", department: "Computer Science" });
      setCapturedImage(null);
      setIsCapturing(true);
    } catch (error) {
      console.error("Registration error:", error);
      setMessage({
        text: error.response?.data?.error || "Failed to register student",
        type: "danger",
      });
    } finally {
      setLoading(false);
